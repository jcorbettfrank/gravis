        ...
        match *event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                if self.dragging {
                    ...
                    self.desired_yaw -= dx * 0.005;
                    self.desired_pitch += dy * 0.005;
                    self.desired_pitch = self.desired_pitch.clamp(-1.55, 1.55);
                }
            ...
            WindowEvent::MouseWheel { delta, .. } => {
                ...
                self.desired_distance *= 1.0 - scroll * 0.1;
                self.desired_distance = self.desired_distance.clamp(0.1, 100.0);
            }
            _ => {}
        }
    }
